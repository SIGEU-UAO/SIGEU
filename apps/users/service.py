...
def save_password_history(user):
    """
    Saves the current user password hash in the history and keeps only the last N.
    ""
    """
    # Marcar anteriores activas como inactivas Mark previous active ones as inactiveontrasenia.objects.filter(idUsuario=user, es_activa=True).update(es_activa=False)
...
class UserService:
    ...
    def registrar(data):
        ...
        # --- Create the record based on the role; if it fails, remove the user ---
        try:
            if rol == "estudiante":
